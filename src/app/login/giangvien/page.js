...
import { setItemOnLocalStorage } from '~/utils/localStorage'
...
function Page() {
  ...
  const handleOnclick = async () => {
    ...
    localStorage.clear()
    try {
      const resquest = await axios.post('http://localhost:8080/api/v1/auth/gv', {
        email,
        password,
      })
      setLoad(false)
      // xử lý lưu user ở đâu đó rồi
      setItemOnLocalStorage('user[gv]', resquest.data)
      // chuyển trang
      router.push('/dashboard/qlmonhoc', { replace: true })
    } catch (e) {
  ...
  return (
    <div className="justify-center items-center w-full shadow rounded-lg bg-white px-6 flex flex-col md:w-1/2 lg:w-1/3 m-auto">
      <h2 className="text-2xl my-4">Đăng nhập[giangvien]</h2>
